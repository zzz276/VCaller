...
class UsernameViewController: UIViewController, WherebyRoomViewControllerDelegate {
    ...
    @IBAction func btnCall(_ sender: Any) {
        ...
        Task {
            ...
            do {
                ...
                signalingManager.callUser(from: currentUserId, to: targetId)
            } catch { print("Error when retrieving other user ID: \(error)") }
        }
        
        targetUsername.text?.removeAll()
    }
    ...
    @objc private func handleRoomCreated(_ notification: Notification) {
        ...
        // FIX: Extract and save the meeting ID so we can delete the room later.
        guard let userInfo = notification.userInfo,
              let meetingId = userInfo["meetingId"] as? String else {
            print("Error: roomCreated notification missing meetingId")
    ...
    func roomViewControllerDidJoinRoom(_ roomViewController: WherebySDK.WherebyRoomViewController) {
        ...
        Task.detached {
            do {
                try await self.db.collection("users").document(currentUserId).setData([ "status": "in-vcall" ], merge: true)
    ...
    func roomViewControllerDidLeave(_ roomViewController: WherebySDK.WherebyRoomViewController) {
        ...
        Task.detached {
            do {
                try await self.db.collection("users").document(currentUserId).setData([ "status": "online" ], merge: true)
